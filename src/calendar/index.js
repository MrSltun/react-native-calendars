...
class Calendar extends Component {
  ...
  addMonth(count) {
    this.updateMonth(this.state.currentMonth.utc().clone().add(count, 'months'));
  }
  ...
  render() {
    ...
    if (current) {
      const lastMonthOfDay = current.utc().clone().add(1, 'months').set('date', 1).add(-1, 'days').format('YYYY-MM-DD');
